# Reyts

**URL del producto**: https://reyts.com  
**Perfil SCF**: https://communityfund.stellar.org/submissions/rect6wxO8IMP2fPLa  
**Fecha de contacto**: —  
**Fecha de respuesta**: —  
**Canal usado**: —  
**Idioma de la comunicación**: Inglés  
**Issue relacionada**: #28

---

## Datos para seguimiento

> Este bloque es obligatorio. Sin él, no se puede cerrar la issue.

**Telegram del contribuidor (vos)**: @Rachaelisa  
**Contacto de la persona del producto**: —

---

## Descripción del producto

Reyts is a peer-to-peer currency exchange platform on Stellar that lets users swap fiat currencies with other users (or merchants) through transparent on-chain settlement. It replaces informal WhatsApp-based exchanges with a structured P2P marketplace, including a WhatsApp bot channel for convenience.

---

## Por qué es candidato para Slice

Reyts connects two strangers exchanging money across borders. Funds move only when both sides fulfill their side of the swap. There is strong bad-faith incentive (payment not sent, wrong amount, impersonation scams — explicitly referenced in their marketing). Meets at least 4 criteria: P2P value exchange, fulfillment-based settlement, strangers, and bad-faith risk.

---

## Respuestas

### Pregunta 1 — ¿Tienen conflictos entre usuarios hoy?

> [Pendiente — por contactar]

---

### Pregunta 2 — ¿Cómo resolvieron la última vez que ocurrió algo así?

> [Pendiente — por contactar]

---

### Pregunta 3 — ¿Hay alguien dedicado a esto? ¿Cuánto les cuesta?

> [Pendiente — por contactar]

---

### Pregunta 4 — ¿Qué es lo más frustrante? ¿Perdieron usuarios o reputación por esto?

> [Pendiente — por contactar]

---

### Pregunta 5 — Si pudieran diseñar la solución ideal, ¿cómo sería?

> [Pendiente — por contactar]

---

### Pregunta 6 — ¿Cuánto valdría resolver esto? ¿Qué modelo de pago preferirían? *(solo si hubo buena señal)*

> [Pendiente — por contactar]

---

## Síntesis

### ¿Tienen disputas hoy?
[Pendiente]

### Cómo las resuelven actualmente (workaround real)
[Pendiente]

### ¿Están pagando por resolver esto hoy?
[Pendiente]

### Costo actual estimado
[Pendiente]

### Dolor funcional
[Pendiente]

### Dolor emocional y social
[Pendiente]

### Visión ideal según ellos
[Pendiente]

### Willingness to pay
[Pendiente]

### Quote más revelador
> [Pendiente]

---

## ¿Es un candidato para Slice?

**Alta** — P2P fiat exchange with escrow-like settlement flows; fraud and non-delivery disputes are inherent to the product category.

---

## Notas adicionales

🔄 In progress — placeholder created for product claim

**Canales de contacto identificados:**
- Email: developer@reyts.com
- Telegram comunidad: t.me/+ht1ba-mctmfjntqx
- Founders: Ayobami Macaulay (COO), Abimbola Adegbite (CSO)
- App activa en Google Play con usuarios reales

**Mensaje de apertura sugerido (inglés):**
> Hey Ayobami, I'm collaborating with Slice, a protocol in the Stellar ecosystem. I'm researching how P2P exchange platforms handle conflicts when a swap goes wrong — I noticed Reyts replaces informal WhatsApp trades with structured settlement, and I'd love to learn how your team handles edge cases. Would you be up for a quick 10-minute chat over text? Happy to share more about what we're building in the ecosystem.
