...
// UltraHonk ZK verifier - disabled until wasm artifact is available
// #[cfg(not(test))]
...
mod tests {
    ...
    fn setup_contract(env: &Env) -> (Address, Address, SliceClient<'_>, Symbol) {
        ...
        (contract_id, token_addr, client, category)
    }
    ...
    fn mint_and_stake(
        ...
        sac_client.mint(juror, &(amount * 10));
        client.stake(juror, &amount);
    }
    ...
    fn create_commit_dispute(
        ...
        let sac_client = test_token::stellar_asset_client(env, token_addr);
        sac_client.mint(&claimer, &200);
        sac_client.mint(&defender, &200);
        ...
        client.pay_dispute(&claimer, &dispute_id, &20).unwrap();
        client.pay_dispute(&defender, &dispute_id, &20).unwrap();
        dispute_id
    }
    ...
    fn dispute_is_queued_only_after_commit_transition() {
        let env = Env::default();
        let (contract_id, token_addr, client, category) = setup_contract(&env);
        ...
        assert!(!queue_after_one.contains(&dispute_id));

        client.pay_dispute(&defender, &dispute_id, &20).unwrap();
        let queue_after_both =
            env.as_contract(&contract_id, || storage::get_draft_queue(&env));
        assert!(queue_after_both.contains(&dispute_id));
    }

    #[test]
    fn random_draw_varies_by_caller_in_same_ledger() {
        let env = Env::default();
        let (_contract_id, token_addr, client, category) = setup_contract(&env);

        let dispute_a = create_commit_dispute(&env, &token_addr, &client, &category, 5);
        let dispute_b = create_commit_dispute(&env, &token_addr, &client, &category, 5);
        ...
        while index_a == index_b {
            attempts += 1;
            assert!(attempts < 20, "unable to generate distinct caller draw index");
            caller_b = Address::generate(&env);
            index_b = draw_queue_index(&env, &caller_b, 2).unwrap();
        }

        mint_and_stake(&env, &token_addr, &caller_a, 100, &client);
        mint_and_stake(&env, &token_addr, &caller_b, 100, &client);

        let expected_a = if index_a == 0 { dispute_a } else { dispute_b };
        let expected_b = if index_b == 0 { dispute_a } else { dispute_b };

        let assigned_a = client.assign_dispute(&caller_a, &category, &20).unwrap();
        let assigned_b = client.assign_dispute(&caller_b, &category, &20).unwrap();
    ...
    fn full_dispute_is_removed_from_queue() {
        let env = Env::default();
        let (contract_id, token_addr, client, category) = setup_contract(&env);
        let dispute_id = create_commit_dispute(&env, &token_addr, &client, &category, 5);

        for _ in 0..5 {
            let juror = Address::generate(&env);
            mint_and_stake(&env, &token_addr, &juror, 100, &client);
            let assigned = client.assign_dispute(&juror, &category, &20).unwrap();
        ...
        mint_and_stake(&env, &token_addr, &extra_juror, 100, &client);
        let result = client.try_assign_dispute(&extra_juror, &category, &20);
        assert!(
            result.is_err() || matches!(result, Ok(Err(_))),
            "assignment should fail when queue is empty"
        );
    }
    ...
    fn expired_dispute_is_pruned_from_queue() {
        ...
        client.pay_dispute(&claimer, &stale_dispute_id, &20).unwrap();
        client.pay_dispute(&defender, &stale_dispute_id, &20).unwrap();

        let active_dispute_id = create_commit_dispute(&env, &token_addr, &client, &category, 5);
        ...
        let queue = env.as_contract(&contract_id, || storage::get_draft_queue(&env));
        assert!(!queue.contains(&stale_dispute_id));
        assert!(queue.contains(&active_dispute_id));
    }
}
